...
(function($) { 
  $(function() {
     ...
     *
     */
    var text = ['DAAN', 'IS', 'A', 'WEEEB', '<span class="glyphicon glyphicon-star"></span>'];
    var show = function(text, index) {
      if(index >= text.length) return;
      $('#home').append($('<div class="line">' + text[index] + '</div>').fadeIn(1000, function(){
        show(text, index+1);
      }));
    }
    show(text, 0);
